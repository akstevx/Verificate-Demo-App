...
import com.verificate.verificate.databinding.FragmentEnterOtpBinding
...
class EnterOtpFragment : BaseFragment() {
    ...
    override fun onCreateView(
        ...
        showNavigationBar(true, "Enter OTP")
    ...
    private fun setObservers() {
        ...
        viewModel.resendPasswordResponse.observeChange(viewLifecycleOwner){
            showSuccess("New one time password has been sent to your mail")
        }

        viewModel.createUserResponse.observeChange(viewLifecycleOwner){
            localStorage.setSignedInStatus(true)
            displaySuccess(successTitle = "Sign up successful", successMessage = "Your account has been \ncreated succesfully"){
    ...
    private fun updateUI(){
        binding.btnGetSecondStep2.setOnClickListener {
            otpCheck()
        }
